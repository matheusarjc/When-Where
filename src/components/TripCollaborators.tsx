...
import { UserPlus, Crown, X, Check, ShieldCheck } from "lucide-react";
...
export function TripCollaborators({
  ...
  const isAdmin =
    isOwner || collaborators.some((c) => c.userId === currentUserId && c.role === "editor");
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        {(isOwner || isAdmin) && (
          <Button
            ...
            variant="outline"
            size="sm"
            aria-label={t("trip.inviteCollaborator", language)}
            className="border-white/10 rounded-full p-2 h-9 w-9 sm:h-auto sm:w-auto sm:rounded-md sm:px-3">
            <UserPlus className="w-4 h-4 mr-0 sm:mr-2" />
            <span className="hidden sm:inline">{t("trip.inviteCollaborator", language)}</span>
          </Button>
        )}
      </div>
        ...
        <div className="flex items-center gap-3 p-3 rounded-lg bg-white/5">
          ...
          <div className="flex-1">
            <p className="text-white">
              {owner?.fullName || "Organizador"}
              {owner?.username && (
                <span className="text-white/40 text-sm ml-2">@{owner.username}</span>
              )}
            </p>
          ...
          <div key={user.id} className="flex items-center gap-3 p-3 rounded-lg bg-white/5">
            ...
            <div className="flex-1">
              <p className="text-white">
                {user.fullName}
                {user.username && (
                  <span className="text-white/40 text-sm ml-2">@{user.username}</span>
                )}
              </p>
              <p className="text-white/40 text-sm">{t(`trip.${user.role}`, language)}</p>
            </div>
            ...
            {/* Actions: promote/demote available for owner and admins (can't change owner) */}
            {isAdmin && user.id !== ownerId && (
              <div className="flex items-center gap-2">
                {user.role !== "editor" ? (
                  <button
                    onClick={() => onPromote && onPromote(user.id)}
                    className="px-2 py-1 rounded-md bg-teal-500/20 text-teal-300 hover:bg-teal-500/30"
                    title="Tornar administrador">
                    <ShieldCheck className="w-4 h-4" />
                  </button>
                ) : (
                  <button
                    onClick={() => onDemote && onDemote(user.id)}
                    className="px-2 py-1 rounded-md bg-white/10 text-white/70 hover:bg-white/20"
