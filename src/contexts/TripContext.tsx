...
export function TripProvider({ children }: { children: ReactNode }) {
  const { trips, setTrips, events, memories, expenses, checklistItems, setChecklistItems, currentUser } = useApp();
  ...
  // Trip actions
  const inviteCollaborator = useCallback((tripId: string, userId: string) => {
  ...
  const acceptInvite = useCallback((tripId: string) => {
    if (!currentUser) return;
  ...
  const declineInvite = useCallback((tripId: string) => {
    if (!currentUser) return;
  ...
  const updateTripCollaborators = useCallback((tripId: string, collaborators: any[]) => {
    const updatedTrips = trips.map((t) =>
      t.id === tripId ? { ...t, collaborators } : t
    );
    setTrips(updatedTrips);
  }, [trips, setTrips]);
  
  // Checklist actions
  const addChecklistItem = useCallback((tripId: string, text: string, category: ChecklistItem["category"]) => {
  ...
  const getTripsByUser = useCallback((userId: string) => {
  ...
  // Export and sharing
  const exportTripToICS = useCallback((tripId: string) => {
    const trip = trips.find((t) => t.id === tripId);
    if (!trip) return;
  ...
  const shareTrip = useCallback((tripId: string) => {
    const trip = trips.find((t) => t.id === tripId);
    if (!trip) return;
